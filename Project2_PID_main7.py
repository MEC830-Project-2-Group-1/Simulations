...
Mr=0.144 #pendulum mass in kg

Mt=Mr+Mp
l_com=((Mp/Mt)*l) + ((Mr/Mt)*l/2) #length of the center of mass of the pendulum system
J = ((Mt) * (l_com)**2) /3  # Moment of inertia of pendulum and point mass

D = 0.01  # damping coefficient of pendulum-cart joint


g = 9.81  # Earths gravity
k_air_pend = 0.001  # Air resistance constant for pendulum total speed chosen arbitrarily
k_air_cart=00.00; #ir resistance of square cart
dt = 0.01  # Essential frame rate of the simulation

# ID controller (found via trial and error)
Kp, Ki, Kd = 3, 00.001 , 0.1
...
def EOM(t, y, F_control):
    ...
    x_ddot_old = (F_imp_cart - F_air_cart + theta_dot**2 * l_com * Mt* D*np.sin(theta)) / Mc
    theta_ddot = (F_imp_pen*l + F_control*l+ Mt*l*g*np.sin(theta) + F_air_linear*l*np.sin(theta) - F_air_angular*l - D*theta_dot + Mc*x_ddot_old*l*np.cos(theta))/J
    ...
    x_ddot = (F_imp_cart - F_friction - F_air_cart + theta_dot**2 * l_com * Mt* np.sin(theta) - theta_ddot*l_com*Mt*np.cos(theta)) / Mc
    theta_ddot = (F_imp_pen*l + F_control*l + Mt*l*g*np.sin(theta) + F_air_linear*l*np.sin(theta) - F_air_angular*l - D*theta_dot + Mc*x_ddot_old*l*np.cos(theta))/J
...
def PID(goal_angle, current_angle,t):
    ...
    if abs(error)<1:
        integral=0 #resetting area term once we get zero area

    derivative = (error - error_old) / (10/200)  # Change in error over change in time
    
    
    
    F_control=F_control_previous + 100*(10/200)*np.clip(Kp * error + Ki * integral + Kd * derivative, -10, 10) #total control signal assuming maximum possible force is 10N
...
def main():
    ...
    for i in range(len(t_eval)): #loops for every solution of "sol" (0-10 seconds w/ 300 points in between) +1 to display all 300 points
        ...
        ax2.plot(sol.t[:i + 1], np.degrees(sol.y[0, :i + 1]), 'b-', label= "Theta (N)")
        ...
        ax4.set_ylim([-3, 1.2])  # Adjust the limits as needed
